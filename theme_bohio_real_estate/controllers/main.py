...
class BohioRealEstateController(http.Controller):
        ...
        for city in cities:
            ...
            if property_count > 0:
                results.append({
                    ...
                    'label': f'<i class="bi bi-geo-alt-fill text-primary"></i> <b>{city.name}</b>, {city.state_id.name}',
        ...
        for state in states:
            ...
            if property_count > 0:
                results.append({
                    ...
                    'label': f'<i class="bi bi-globe text-info"></i> {state.name} <small class="text-muted">(Departamento)</small>',
                    'property_count': property_count,
                    'priority': 2,
                    'state_id': state.id,
                })
            ...
            for region in regions:
                ...
                if property_count > 0:
                    results.append({
                        ...
                        'label': f'<i class="bi bi-house-fill text-success"></i> {region.name} <small class="text-muted">({region.city_id.name})</small>',
                        'property_count': property_count,
                        'priority': 1,
                        'region_id': region.id,
            ...
            for project in projects:
                ...
                if property_count > 0:
                    ...
                    results.append({
                        ...
                        'label': f'<i class="bi bi-building text-warning"></i> <b>{project.name}</b> <small class="text-muted">({location_text})</small>',
                        'property_count': property_count,
                        'priority': 2,
    ...
    def api_properties_proyectos(self, limit=4, **kwargs):
        """
        Endpoint específico para PROYECTOS (no propiedades individuales)
        Muestra proyectos con propiedades disponibles
        """
        _logger.info(f"[HOMEPAGE] Cargando {limit} proyectos")
        ...
        # OPTIMIZACIÓN: search_read de proyectos
        ...
        # Para cada proyecto, contar propiedades disponibles
        serialized = []
        base_url = request.httprequest.url_root.rstrip('/')

        for project in projects_data:
            # Contar propiedades disponibles en el proyecto
            available_count = Property.search_count([
                ('project_worksite_id', '=', project['id']),
                ('is_property', '=', True),
                ('active', '=', True),
                ('state', '=', 'free'),
            ...
            # Solo mostrar proyectos con propiedades disponibles
            if available_count == 0:
                continue

            # Imagen del proyecto
            image_data = project.get('image_1920')
            if image_data:
                if isinstance(image_data, str):
                    image_url = f"data:image/jpeg;base64,{image_data}"
                else:
                    image_url = f"{base_url}/web/image/project.worksite/{project['id']}/image_1920"
            else:
                image_url = f"{base_url}/theme_bohio_real_estate/static/src/img/placeholder.jpg"
            ...
            serialized.append({
                ...
                'code': project.get('default_code', ''),
                'city': project.get('city', ''),
                'state': state_name,
                'neighborhood': '',  # Proyectos no tienen barrio
                'area': 0,  # Proyectos no tienen área individual
                'bedrooms': 0,
                'bathrooms': 0,
                'price': 0,  # Proyectos no tienen precio individual
                'type_service': 'sale',
                'image_url': image_url,
                'url': f"/proyecto/{project['id']}",
                'project_id': project['id'],
                'project_name': project.get('name', ''),
                # Datos adicionales del proyecto
                'available_units': available_count,
                'total_units': project.get('no_of_property', 0),
                'description': project.get('description', ''),
            })

        _logger.info(f"[HOMEPAGE] Encontrados {len(serialized)} proyectos con unidades disponibles")

        return {
            'success': True,
            'properties': serialized,  # Devolver proyectos serializados
...
**Fecha:** {fields.Datetime.now()}
        ...
        try:
            ...
            for prop in properties:
                ...
                markers.append({
                    ...
                    'url': f'/property/{prop.id}',
                    'pin_color': pin_color,
                    'icon': self._get_property_icon_map(prop.property_type),
                })
    ...
    def _get_property_icon_map(self, property_type):
    ...
    # ========================================================================
    # CONSOLIDACIÓN: Métodos fusionados desde map_controller.py
    # ========================================================================
    ...
    # ========================================================================
    # CONSOLIDACIÓN: Métodos fusionados desde property_banner.py
    # ========================================================================
