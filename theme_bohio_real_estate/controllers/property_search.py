...
class PropertySearchController(http.Controller):
    ...
    def _serialize_properties(self, properties, search_context):
        ...
        for prop in properties:
            ...
            # En Odoo 18, selection puede ser una función que recibe el modelo
            property_type_label = ''
            if prop.property_type:
                selection_field = prop._fields['property_type']
                if callable(selection_field.selection):
                    # La función selection recibe el modelo, no el env
                    selection_list = selection_field.selection(prop)
                else:
    ...
    def _autocomplete_cities(self, term, search_context, limit):
        ...
        for city in cities:
            ...
            results.append({
                ...
                'label': f'<i class="bi bi-geo-alt-fill text-primary"></i> <b>{city.name}</b>, {city.state_id.name if city.state_id else ""}',
    ...
    def _autocomplete_regions(self, term, search_context, limit):
        ...
        for region in regions:
            ...
            results.append({
                ...
                'label': f'<i class="bi bi-house-fill text-success"></i> {region.name} <small class="text-muted">({region.city_id.name if region.city_id else ""})</small>',
                'property_count': property_count,
        ...
        for project in projects:
            ...
            if property_count > 0:
                results.append({
                    ...
                    'label': f'<i class="bi bi-building text-warning"></i> {project.name} <small class="text-muted">(Proyecto)</small>',
                    'property_count': property_count,
                    'priority': 2,
    ...
    def _autocomplete_properties(self, term, search_context, limit):
        ...
        for prop in properties:
            results.append({
                ...
                'label': f'<i class="bi bi-key text-info"></i> {prop.default_code or ""} - {prop.name}',
                'property_count': 1,
                'priority': 1,
                'property_id': prop.id,
            })
