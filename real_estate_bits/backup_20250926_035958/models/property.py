...
class Property(models.Model):
    ...
    latitude = fields.Float(string='Latitud', digits=(10, 8), tracking=True, index=True)
    longitude = fields.Float(string='Longitud', digits=(11, 8), tracking=True, index=True)
        ...
        for order in self:
            if order.partner_id and not (order._origin.id and order.user_consing_id):
                order.user_consing_id = (
                    ...
                    or (self.user_has_groups('sales_team.group_sale_salesman') and self.env.user.partner_id)
                )
    ...
    def write(self, vals):
        ...
        if any(field in vals for field in ['street', 'street2', 'zip', 'city', 'city_id', 'state_id', 'country_id']):
