...
class DynamicCongifTool(http.Controller):
    ...
    def config_product_data(self, **post):
        ...
        if request.session.get('website_sale_pricelist_time', 0) < now - 60*60:      
            pricelist = website.pricelist_id
    ...
    def get_dynamic_config(self, **post):
        ...
        if templatecheck:
            if base_config_domain and base_config_domain[0]:
                if model == 'product.template':
                    full_domain = base_config_domain[0] + [('is_published', '=', True)]
                    records = request.env[model].sudo().search(full_domain, limit=record_limit, order=record_orderby)
                elif model: 
                    Model = request.env[model].sudo()
                    model_fields = Model._fields

                    valid_sortby = sortby if sortby in model_fields else False
                    valid_orderby = orderby if orderby in model_fields else False

                    # Construct record_orderby safely
                    if valid_sortby and valid_orderby:
                        record_orderby = f"{valid_sortby},{valid_orderby}"
                    elif valid_sortby:
                        record_orderby = valid_sortby
                    elif valid_orderby:
                        record_orderby = valid_orderby

                    try:
                        records = Model.search(
                            base_config_domain[0], limit=record_limit, order=record_orderby
                        )
                    except Exception as e:
                        records = Model.search(base_config_domain[0], limit=record_limit)
                else: 
...
# Fixed the wishlist count
class BizopleWebsiteSaleWishlist(WebsiteSaleWishlist):

    @http.route('/shop/wishlist/add', type='json', auth='public', website=True)
    def add_to_wishlist(self, product_id, **kw):
        res = super(BizopleWebsiteSaleWishlist, self).add_to_wishlist(product_id, **kw)
        
        new_count = len(request.env['product.wishlist'].current())
        return {
            'new_count': new_count,
        }
